    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className={`relative bg-(--bg-2) rounded-lg shadow-lg max-w-lg w-full max-h-[90vh] overflow-y-auto ${className}`}>
                {title && (
                    <div className="flex items-center justify-between p-6 max-md:p-4 max-sm:p-3 border-b border-(--border-1)">
                        <h2 className="text-2xl max-md:text-xl max-sm:text-lg font-bold text-(--text-1)">{title}</h2>
                        <button
                            onClick={onClose}
